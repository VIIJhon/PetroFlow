"""
Unit and Integration Tests for PVT Engine and FTA Engine in PetroFlow Enterprise
...
SQLALCHEMY_TEST_DATABASE_URL = "sqlite:///./test_pvt_fta.db"
...
# ==========================================
# 2. FTA ENGINE UNIT TESTS
# ==========================================

def test_fta_solve_tree():
    # Define a simple fault tree:
    # Top event (OR) -> Gate A (AND), Gate B (BASIC, prob=0.1)
    # Gate A (AND) -> Basic 1 (prob=0.2), Basic 2 (prob=0.3)
    nodes = {
        "top": {
            "id": "top",
            "name": "Falla Total",
            "type": "OR",
            "children": ["gate_a", "basic_b"]
        },
        "gate_a": {
            "id": "gate_a",
            "name": "Falla Mecánica",
            "type": "AND",
            "children": ["basic_1", "basic_2"]
        },
        "basic_b": {
            "id": "basic_b",
            "name": "Falla Humana",
    ...
    results = FTAEngine.solve_tree(nodes, "top")
    
    # Expected prob calculations:
    # P(gate_a) = 0.2 * 0.3 = 0.06
    # P(top) = 1 - (1 - P(gate_a)) * (1 - P(basic_b)) = 1 - (1 - 0.06) * (1 - 0.1)
    # P(top) = 1 - 0.94 * 0.9 = 1 - 0.846 = 0.154
    
    assert abs(results["top_event_probability"] - 0.154) < 1e-5
    assert results["solved_nodes"]["gate_a"]["probability"] == 0.06
    assert len(results["importance"]) == 3
...
def test_api_fta_endpoint(client, auth_headers):
    # Tests the POST /fta endpoint
    nodes = {
        "top": {
            "id": "top",
            "name": "Top Failure",
    ...
    response = client.post("/api/v2/reliability/fta", json=payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert "top_event_probability" in data
    # P(top) = 1 - 0.95 * 0.92 = 1 - 0.874 = 0.126
    assert abs(data["top_event_probability"] - 0.126) < 1e-5
